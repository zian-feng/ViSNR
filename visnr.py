# -*- coding: utf-8 -*-
"""FinalFinalViSNR.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1HPZxglyLM4EH3UwPUhxHgdYMASHhZTcB
"""
...
class AUG(Dataset):
    def __init__(self, imgs: torch.Tensor, msks: torch.Tensor):

        self.x = imgs           # [1000, 3, 512, 512]
        self.y = msks           # [1000, 1, 512, 512]


        self.aug = v2.Compose([
            v2.RandomHorizontalFlip(p = 0.2),
            v2.RandomVerticalFlip(p = 0.2),
            v2.RandomRotation(degrees= [-15, 15], interpolation = InterpolationMode.NEAREST, expand = False),
            v2.RandomResizedCrop(size = (224, 224), scale = (1.0, 1.4), ratio = (1.0, 1.0),
    ...
    def __getitem__(self, idx):
        ...
        # aug_xy = (lambda imgs, msks: imgs, msks,= self.aug() for imgs, msks in self.xy)
...
class SNR_ATTN(ViTSelfAttention):
    def __init__(self, config):
        ...
        # snr params
        self.snr_bias_w = nn.Parameter(torch.tensor(1.0))           # initialize to 1 -- learnable weights
        self.snr_scaler = nn.Parameter(torch.tensor(1.0))           # initialize to 1
    ...
    def forward(self, hidden_states, pixel_values = None, snr_values = None):
        ...
        A = self.num_attention_heads                              # number of attention heads
        # D = int(768 / 12)
        ...
        if snr_values is not None:
            ...
            cls_tokens = torch.zeros(B, 1, device = snr_values.device, dtype = snr_values.dtype)
...
class ViSNR(nn.Module):
    ...
    def compute_patch_snr(self, pixel_values, patch_size = 16, epsilon = 1e-8):
        ...
        patches = pixel_values.unfold(2, patch_size, patch_size)      # [B, C, grid_H, W, patch_size]
        patches = patches.unfold(3, patch_size, patch_size)           # [B, C, grid_H, grid_W, patch_size, patch_size]
        # reshaping patches to dim [B, C, N, p, p] 5D tensor
        patches = patches.contiguous().view(B, C, -1, patch_size, patch_size)   # [B, C, N, p, p]
    ...
    def send_to_attn_layer(self, snr_vals):
        ...
        for block in self.vit.encoder.layer:
            if hasattr(block.attention, 'attention') and isinstance(block.attention.attention, SNR_ATTN):
...
targs1 = TrainingArguments(
    output_dir = 'training/visnr_aug/p1',                     # separate training output
...
targs2 = TrainingArguments(
    output_dir = 'training/visnr_aug/p2',                     # separate training output
    # logging_dir = 'training/vit-b16/logs',
    report_to = ['none'],
    num_train_epochs = 135,                               # training decoder, use more epochs
    per_device_train_batch_size = 25,
    per_device_eval_batch_size = 25,
